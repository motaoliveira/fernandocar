<?php
	...
	@mkdir("../clientes/$id/", 0777, true);
	@mkdir("../clientes/$id/doc", 0777, true);
	$dir_upload = "../clientes/$id/doc/";
	$max_size = 2000000;
...
if ($_FILES['upload']['size'] <= $max_size && $_FILES['upload']['size'] > 0) {
	$new_name = $namefile;
	$copied = copy($_FILES['upload']['tmp_name'], $dir_upload . $new_name);
	$nomefiledb = $new_name.'.'.$extensao_final;

	//print($copied);
	$query = "INSERT INTO `clientefile` (`id`, `nome`, `cargo`, `documento`, `view`, `obs`, `cliente`) VALUES (NULL, '$nome', '$cargo', '$nomefiledb', '1', 'obs', '$Client')";
